...
# 3. Preprocessing with tensor conversion handling
def preprocess_function(examples):
    ...
    try:
        ...
        for i in range(len(inputs["input_ids"])):
            sample_idx = inputs["overflow_to_sample_mapping"][i].item()  # Convert to Python int
        ...
        if "token_type_ids" in inputs:
            inputs_dict["token_type_ids"] = inputs["token_type_ids"].tolist()
...
# 4. Dataset preparation with smaller batches
try:
